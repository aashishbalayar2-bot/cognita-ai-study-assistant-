        ...
        try {
            ...
            sessionPromiseRef.current = ai.live.connect({
                ...
                config: {
                    ...
                    systemInstruction: `You are "Jawz", an AI study assistant, helping the student revise from the provided documents: ${fileNames}. Engage in a spoken conversation.`
                },
                callbacks: {
                    onopen: () => {
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex-none p-4 bg-white border-2 border-slate-200 rounded-2xl flex justify-between items-center mb-4 shadow-sm">
                <div>
                    <h3 className="text-lg font-extrabold text-slate-800">Live Spoken Revision</h3>
            ...
            <div className="flex-1 bg-slate-50 border-2 border-slate-200 rounded-2xl p-4 overflow-y-auto space-y-4">
                ...
                {transcripts.map((t, i) => (
                    <div key={i} className={`p-4 rounded-2xl max-w-[80%] font-medium shadow-sm ${t.type === 'user' ? 'bg-sky-500 text-white self-end ml-auto rounded-br-none' : 'bg-white text-slate-700 border-2 border-slate-200 self-start rounded-bl-none'}`}>
                       <span className="font-extrabold uppercase text-xs block mb-1 opacity-70">{t.type}</span>{t.text}
                    </div>
                ))}
            </div>
        </div>
